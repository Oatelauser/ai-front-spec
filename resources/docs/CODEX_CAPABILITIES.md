...
维护版本：3.1.0。外部来源沿用原清单，实际安装前须核对当前宿主和上游；本文不代表本轮已重新联网核验。
...
- **Figma 模式**：UI 模式 + Figma 插件；连接仅在当前任务需要且已授权时执行。
- **GitHub 远端模式**：所需模式 + GitHub 插件；连接按当前任务需要及授权执行。
- **完整模式**：以上全部及完整清单中的可选 Skill。只有用户明确要求才使用；默认按需，缺少任务上下文时推荐核心模式。

安装模式只决定候选依赖范围，不代表已可用或每个任务都加载全部能力。执行时按任务路由选择最小组合。审计、批准输入、状态记录与发现性检查使用 [$codex-capability-setup](../.agents/skills/codex-capability-setup/SKILL.md)；目标状态使用 `docs/capability-state.json`，工具包不保存项目安装状态。
