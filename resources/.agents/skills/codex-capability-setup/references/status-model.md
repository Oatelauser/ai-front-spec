...
记录文件为当前项目 `docs/capability-state.json`，模板为 `.codex/templates/capability-state.template.json`。B/C 阶段获得记录授权后可在本仓库创建或更新，保留已有条目；不把实际能力状态写回模板。使用 JSON 解析和保留未知字段的定向合并，不用字符串拼接更改记录。
