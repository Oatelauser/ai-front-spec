...
After an explicit selection, immediately enter the project-profile grilling interview. Scan the selected template and the materialized `docs/PROJECT_PROFILE.md` for every `<待填写...>` placeholder. Ask the current decision frontier in numbered rounds with a recommended answer, then wait for the user's response before asking the next round; every round must show the remaining frontier. Do not stop after reporting only high-impact fields: low-impact placeholders must be answered, marked `无`/`不适用`, or recorded as `deferred` with a reason, impact, and follow-up condition.
