...
3. If profile facts, `deliveryTargets`, or component-catalog facts are missing, draft, deferred, or conflicted, identify whether the gap has high impact. Prompt the user to run `$project-profile` or provide a task-scoped override. Never force profile completion and never silently invent framework, route, API, permission, target, component variant, or acceptance facts.
