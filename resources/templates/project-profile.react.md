...
本模板提供 React + TypeScript 前端项目的画像候选，在当前仓库生成 `docs/PROJECT_PROFILE.md` 时，以实际选定的框架、版本、代码、配置、接口和测试替换预设与 `<待填写：...>`。尚无工程时先建立草案，在当前根目录搭建后补充事实，再启用严格 AI 指引门禁。未生成 `.codex/scripts/` 副本时，将本画像的指引命令改为 `node resources/scripts/check-ai-guidance.mjs --root .`，总门禁中的路径同步调整。
