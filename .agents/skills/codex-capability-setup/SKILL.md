...
Use [status model](references/status-model.md) for audit output and approved state updates in the current repository's `docs/capability-state.json`, including repositories that still contain template resources. Phase A remains read-only. Never infer discoverability from exit code alone. Project rule initialization belongs to $bootstrap-project; capability setup does not create project rules, business code, or external objects.
