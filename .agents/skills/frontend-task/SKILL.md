...
3. Apply [shared execution](references/common-workflow.md), including evidence priorities and existing authorization. Complete unresolved design/API confirmation points before dependent code; do not repeatedly ask about already confirmed decisions.
4. Use project commands and relevant TDD/security/API/UI skills. Validate with [acceptance matrix](references/acceptance-matrix.md) and report using [acceptance evidence](../../../docs/AI_ACCEPTANCE_EVIDENCE.md). Missing Browser evidence remains unverified, even when screenshots or static checks pass.
