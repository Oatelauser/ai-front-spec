...
读取项目级路由、画像、前端约定、任务合同和组件目录；检查工作区、目标路由、相邻实现、测试和真实命令；识别 taskType（new-page/incremental/bug-fix/refactor）；识别并记录 screenshot/prototype/html/figma/api/requirement 来源的路径或 URL、版本、读取时间、视口和可访问性；检测来源冲突、缺失、版本变化、画像缺口和所需专项 Skill。
...
形成 PLAN.md 时记录任务类型、来源采用/排除、目标路由、文件范围、保留行为、UI/API/状态方案、专项 Skill、验收矩阵、风险、override、fingerprint 和失效条件。高影响决定未确认时保持 awaiting-confirmation，不进入依赖它的实现。
```
...
```text
仅在 plan 有效、必要高影响决定已确认且当前范围已获授权时写入代码。复用项目已有组件、路由、样式、请求入口和测试；将 mock、假设和 user_override 限定在当前 task-id。发现计划与实际代码冲突时，独立部分可继续，依赖冲突部分暂停并生成 plan 差异，不自动扩大范围。
...
```text
依据 taskType、来源和影响范围选择 acceptance matrix，而不是机械执行固定命令。运行仓库中真实存在且适用的测试、类型检查、lint、构建、Browser、视觉对比和接口验证；区分通过、失败、跳过、未运行和不适用；区分真实联调、mock 和未验证。记录复现/回归、loading/empty/error/unauthorized/disabled/success 等适用状态及证据。
```

### `report`

```text
生成完整交付报告：目标与 taskType；采用、未采用和冲突来源及原因；实际修改文件；计划偏差；实际命令和验证结果；未验证项；mock、假设和 task-scoped override；项目画像状态；当前 taskStatus；后续待决定事项。构建成功、HTTP 200、静态检查或 mock 不得单独表述为业务验收完成。
```
