...
本模板用于 React 18 + TypeScript + Vite 的前端项目。复制到目标项目后，必须以真实代码、配置、接口契约和测试为准填写所有 `<待填写：...>`；未确认的事实不得猜测。填写完成并通过检查后，才能启用严格 AI 指引门禁。
...
| AI 指引校验 | `node scripts/check-ai-guidance.mjs` |
| 总质量门禁 | `node scripts/check-ai-guidance.mjs --strict && pnpm lint && pnpm type-check && pnpm test && pnpm build` |
