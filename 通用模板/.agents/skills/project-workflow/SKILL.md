...
4. 任务需要外部能力时，读取 [能力安装清单](../../../docs/AI_CAPABILITY_REQUIREMENTS.md)，
   核对插件、宿主能力和独立 Skill；未安装时按授权边界安装或报告缺失。
5. 只在触发条件适用时加载详细文档：
   - 通用复杂任务：[任务提示词](../../../docs/AI_TASK_PROMPT.md)。
   - 新增页面、重做布局或显著交互变更：[页面提示词](../../../docs/AI_PAGE_PROMPT.md) 和
     [组件目录](../../../docs/AI_COMPONENT_CATALOG.md)。
   - 接口、身份、权限、数据、AI 责任或安全：[项目研发规范](../../../docs/AI_PROJECT_STANDARDS.md)。
   - 提示词、Skill、路由或代理工作流：[提示词工程](../../../docs/AI_PROMPT_ENGINEERING.md)。
6. 分别声明业务、视觉和工程事实源。外部输出只能补充，不能覆盖项目事实。
7. 若项目画像仍有影响本次任务的占位符，只输出参数化计划、信息缺口和继续条件，不进入具体文件设计或实现。
...
2. 提示词或 Skill 变更运行 `node scripts/check-ai-guidance.mjs` 和当前宿主的 Skill 校验器。
