...
Vendored 18 项（许可与修改记录见仓库根 `NOTICE`；更新方式一律为**重新 vendor**：从「上游」列取最新快照覆盖 `.agents/skills/<目录>`，然后运行 `node .toolkit/scripts/sync-mirror.mjs` 重建镜像）：
...
| `grill-me` / `prototype` | claude-plugins-official/mattpocock-skills 1.2.3 | 2026-09-25 快照，MIT；grill-me 原名 grilling（frontmatter 对齐目录名）；prototype 支持无视觉参照新页面的方向原型硬门槛 |
