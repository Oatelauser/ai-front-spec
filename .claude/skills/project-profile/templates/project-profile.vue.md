...
本模板提供 Vue + TypeScript 前端项目的画像候选，在当前仓库生成 `docs/PROJECT_PROFILE.md` 时，以实际选定的框架、版本、代码、配置、接口和测试替换预设与 `<待填写：...>`。尚无工程时先建立草案，在当前根目录搭建后补充事实，再启用严格 AI 指引门禁。指引校验使用 `node .toolkit/scripts/check-ai-guidance.mjs --root .`。

模板选择初始状态为 `pending`；Vue、版本、目录和命令均为候选，不是已确认事实。首次使用 `$project-profile` 必须展示 `generic`、`react`、`vue` 并由用户选择或暂缓，未填写字段不得静默补成事实。

组件目录由 `$project-profile components` 单独初始化。Vue 项目的推荐规划基线为 Element Plus；它必须由用户明确选择后才能写入 `docs/rules/AI_COMPONENT_CATALOG.md`，且规划候选不等于已实现组件。
...
| 工程事实 | `代码与配置 > docs/rules/FRONTEND_CONVENTIONS.md > 本文件第 3、4 节` | `同时服从系统约束、用户本次要求、AGENTS.md 和可信接口契约；工程事实冲突且无法安全兼容时停下提问，不擅自二选一。` |
...
- 应用框架：`Vue 3.4+ / Vite 5 / Vue Router 4，纯客户端 SPA（若实际为 SSR、SSG 或混合渲染必须改写）；统一 <script setup lang="ts">，禁用 Options API`
- UI 与样式系统：`Element Plus（按需引入）+ SCSS + CSS Variables，组件样式使用 scoped 与 BEM；禁用 Tailwind 及其他原子类方案；图标仅使用项目批准的 iconfont Symbol 入口`
- 状态、数据与请求层：`Pinia（按模块拆 store）+ axios 二次封装；DTO → VO 仅在 src/api/<模块>/adapter.ts 转换；请求边界执行运行时校验、错误归一化、取消和安全重试规则`
- Schema/接口生成：`默认无自动生成；类型手写于 src/api/<模块>/types.ts，与 docs/api/<模块>.md 一一对应；若后端提供 OpenAPI，应明确生成源、生成命令和禁止手改目录`
- 测试框架：`Vitest + @vue/test-utils；接口 mock 按项目批准方案（如 MSW）；关键用户流程使用 Playwright 或项目批准的等价 E2E 工具`
- 源码目录：`src/`
- 公共组件目录：`src/components/（含 SvgIcon 或项目批准的图标封装）`
- 业务域目录：`src/views/<模块>/{index.vue, components/, config.ts, hooks/}，src/api/<模块>/{index.ts, types.ts, adapter.ts}；store、router 和 composables 目录按实际项目补充`
- 请求统一入口：`src/utils/request.ts`
- 生成代码目录：`无；如存在，填写真实路径、生成命令和禁止手工修改规则`
- 国际化入口：`src/locales/index.ts（vue-i18n；语言包 src/locales/{zh-CN,en-US}/<模块>.ts；持久化 key app-locale；补充 fallback、懒加载和格式化规则）`
- 主题入口：`src/styles/theme/{tokens.scss, light.scss, dark.scss}（html[data-theme] 切换；持久化 key app-theme；同步 Element Plus dark 模式并处理首屏闪烁）`
- 前端约定入口：`docs/rules/FRONTEND_CONVENTIONS.md（图标、国际化、主题 token、VO/adapter、配置化字段、四态、代码卫生和文档产物）`

## 5. 硬边界

- 必须复用的基础设施：`src/components/SvgIcon、src/utils/request.ts、src/styles/theme/tokens.scss、src/locales、src/constants；页面 loading、empty、error、unauthorized 四态统一实现方式见 docs/rules/FRONTEND_CONVENTIONS.md`
- 禁止引入或复制的能力：`任何未经批准的图标 npm 包（lucide、heroicons、@element-plus/icons-vue 等）；第二套 UI 库；Tailwind / CSS-in-JS；Google Fonts；未经批准的外链图片；硬编码用户文案与色值；any；emoji。测试或构建工具如需例外，必须在本画像登记。`
...
| 总质量门禁 | `node .toolkit/scripts/check-ai-guidance.mjs --root . --strict && pnpm lint && pnpm type-check && pnpm test && pnpm build` |
...
| 代码托管 | `GitHub 插件仅在用户明确要求远端读取或写入时使用；复杂需求的外部案例检索不得替代项目事实` |
| 发布部署 | `<待填写：部署工具或“无”；发布必须有明确授权和回滚证据>` |
| Vue 框架最佳实践 | `Vue 官方文档、项目 ESLint/TypeScript 规则和已确认工程约定；不把 React 专属规则直接套用到 Vue`
