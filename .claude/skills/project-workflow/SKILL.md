...
---
<!-- AUTO-GENERATED from .agents/skills/project-workflow. DO NOT EDIT. Run: node .toolkit/scripts/sync-mirror.mjs -->

# Project Workflow

## 建立项目事实

1. 读取 [AGENTS.md](../../../AGENTS.md)、[项目画像](../../../docs/PROJECT_PROFILE.md) 和 [组件目录](../../../docs/rules/AI_COMPONENT_CATALOG.md)；画像维护按 `$project-profile profile` → `$project-profile components` 顺序执行。
2. 检查本次需求、最近实现、邻近测试、配置和已有组件，再决定改动。
3. 读取 [任务路由](references/task-routing.md)，选择能完成任务的最小 Skill、插件和工具组合。
4. 任务需要外部能力时，读取 [能力清单](../../../docs/capabilities.md)，核对插件、宿主能力和独立 Skill；未安装时按授权边界安装或报告缺失。
5. 只在触发条件适用时加载详细文档：
   - 通用复杂任务：[任务合同](../../../docs/rules/AI_TASK_CONTRACT.md)。
   - 改动落在页面或视图组件文件（`.vue`/`.tsx`/`.jsx` 等）：[前端任务规范](../../../docs/rules/AI_FRONTEND_TASK.md) 和 [组件目录](../../../docs/rules/AI_COMPONENT_CATALOG.md)。
   - 接口、身份、权限、数据、AI 责任或安全：[项目研发规范](../../../docs/rules/AI_PROJECT_STANDARDS.md)。
   - 提示词、Skill、路由或代理工作流：[工作流原则](../../../docs/rules/AI_WORKFLOW_PRINCIPLES.md)。
...
2. 提示词或 Skill 变更运行 `node .toolkit/scripts/check-ai-guidance.mjs --root .` 和当前宿主的 Skill 校验器。
...
6. 使用 [验收证据模板](../../../docs/rules/AI_ACCEPTANCE_EVIDENCE.md) 交付实际证据和未验证项。
